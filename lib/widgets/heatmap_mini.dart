...
class HeatmapMini extends StatefulWidget {
  ...
  @override
  State<HeatmapMini> createState() => _HeatmapMiniState();
}
...
class _HeatmapMiniState extends State<HeatmapMini>
  ...
  Color _getColor(double intensity) {
    if (intensity > 0.7) return const Color(0xFFEF4444);
    if (intensity > 0.4) return const Color(0xFFF97316);
    if (intensity > 0.2) return const Color(0xFFEAB308);
...
class _InsoleHeatmapPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    // ViewBox: 280 20 240 580, rotated 180°
    ...
    canvas.translate(-280 * scaleX, -20 * scaleY);

    // Parse and draw the exact CAD path
    final path = _parseSVGPath(_cadPath, scaleX, scaleY);
    ...
    // Draw pressure zones
    final zones = [
      {'cx': 388.0, 'cy': 520.0, 'rx': 28.0, 'ry': 24.0, 'data': sensorData[0]}, // Heel
      {'cx': 360.0, 'cy': 330.0, 'rx': 22.0, 'ry': 32.0, 'data': sensorData[1]}, // Mid
      {'cx': 400.0, 'cy': 150.0, 'rx': 32.0, 'ry': 36.0, 'data': sensorData[2]}, // Ball
      {'cx': 400.0, 'cy': 65.0, 'rx': 28.0, 'ry': 22.0, 'data': sensorData[3]}, // Toes
    ];

    for (final zone in zones) {
      final intensity = zone['data'] as double;
      final rx = (zone['rx'] as double) + (isActive ? intensity * 5 : 0);
      final ry = (zone['ry'] as double) + (isActive ? intensity * 5 : 0);

      final paint = Paint()
        ..color = getColor(intensity).withOpacity(isActive ? intensity * 0.9 : 0.3)
        ..maskFilter = const MaskFilter.blur(BlurStyle.normal, 12);

      canvas.drawOval(
        Rect.fromCenter(
          center: Offset((zone['cx'] as double) * scaleX, (zone['cy'] as double) * scaleY),
          width: rx * 2 * scaleX,
          height: ry * 2 * scaleY,
        ),
        paint,
    ...
    for (final zone in zones) {
      canvas.drawCircle(
        Offset((zone['cx'] as double) * scaleX, (zone['cy'] as double) * scaleY),
        3,
        sensorPaint,
      );
    }
  ...
  Path _parseSVGPath(String pathData, double scaleX, double scaleY) {
    final path = Path();
    final commands = pathData.split(RegExp(r'(?=[MLHVCSQTAZ])', caseSensitive: false));
        ...
        case 'L':
          for (int i = 0; i < coords.length; i += 2) {
            if (i + 1 < coords.length) {
              currentX = coords[i] * scaleX;
              currentY = coords[i + 1] * scaleY;
  ...
  bool shouldRepaint(covariant _InsoleHeatmapPainter oldDelegate) {
    return oldDelegate.sensorData != sensorData || oldDelegate.isActive != isActive;
  }
}
